...
def my_own_visualization_function_with_not_a_better_name_than_the_one_you_came_up_with(x, y, ax, ax_histx, ax_histy):
	"""
	SCATTER PLOT WITH HISTOGRAMS BY:
	https://matplotlib.org/stable/gallery/lines_bars_and_markers/scatter_hist.html#sphx-glr-gallery-lines-bars-and-markers-scatter-hist-p
	:return:
	"""
	...
	bins_x = np.arange(np.min(x), np.max(x), .5)
	bins_y = np.arange(np.min(y), np.max(y), .5)
	ax_histx.hist(x, bins=bins_x)
	ax_histy.hist(y, bins=bins_y, orientation='horizontal')



def your_own_visualization_function_but_with_a_better_name(data, col_x, col_y):
	"""
	:param data:
	:param col_x:
	:param col_y:
	...
	x = data[:, col_x]
	y = data[:, col_y]
	my_own_visualization_function_with_not_a_better_name_than_the_one_you_came_up_with(x, y, ax, ax_histx, ax_histy)
...
def main( argv ):
	...
	data, headers, title = read_csv("student-mat.csv")
	...
	scatter(data, headers, col_x=0, col_y=13, col_c=class_col, title=title)	# pick any X and Y columns you want
	#pair_plot( data, headers, class_col=class_col, title=title )
	#heatmap( data, headers, title=title )
	your_own_visualization_function_but_with_a_better_name(data, 0, 13)
