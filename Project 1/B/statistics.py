...
Execution:		>> python3 statistics.py [path_teo_dataset, str] [class_column_header, str]
...
def scatter_gauss(data, headers, col_x, col_y, col_c=None, title="Gaussian Distribution"):
    ...
    if colorcoded:
        ...
        for i in range(num_classes):
            ...
            mn = np.mean(data_xy[has_lbl, :], axis=0)
            cv = np.cov(data_xy[has_lbl, :], rowvar=False)
            ax.plot(mn[0], mn[1],  'dk', markerfacecolor=color, markersize=15, alpha=0.5, label=f"{label} mean")
            ...
            for n_std in range(1,4):
                gaussian_ellipse( mn, cv, ax, n_std, color=color )
        ...
        mn = np.mean(data_xy, axis=0)
        cv = np.cov(data_xy, rowvar=False)

        ax.plot(mn[0], mn[1], 'dk', markerfacecolor="blue", markersize = 15, alpha = 0.5, label = f"t mean" )
        # 1. compute the mean
        # 2. compute the covariance matrix
        # 3. plot the mean as a large diamond, e.g. print ax.plot( ?, ?, 'dk', markerfacecolor=color, markersize=15, alpha=0.5, label=f"{label} mean" )
        # 4. Visualize distribution shape by drawing ellispses at the 1st, 2nd, and 3rd standard deviations
        for n_std in range(1,4):
            gaussian_ellipse( mn, cv, ax, n_std, color="blue" )
...
def report_stats(data, headers, col_c=None):
    ...
    val_median, val_mean, val_std, val_var, val_cov = np.median(data, axis=0), np.mean(data, axis=0),\
                                                      np.std(data, axis=0), np.var(data, axis=0), np.cov(data, rowvar=False)
    ...
    print(f"Std Dev: \t{val_std}")
    print(f"Variance:\t{val_var}")
    print(f"Cov:     \n{val_cov}\n")
    ...
    if (col_c != None) and (col_c < data.shape[1]):
        ...
        for i in range(num_classes):
            ...
            print(f"Min:     \t{np.min(data[has_lbl, :], axis=0)}")
            print(f"Max:     \t{np.max(data[has_lbl, :], axis=0)}")
            print(f"Range:   \t{(np.ptp(data[has_lbl, :], axis=0))}")
            print(f"Median:  \t{np.median(data[has_lbl, :], axis=0)}")
            print(f"Mean:    \t{np.mean(data[has_lbl, :], axis=0)}")
            print(f"Std Dev: \t{np.std(data[has_lbl, :], axis=0)}")
            print(f"Variance:\t{np.var(data[has_lbl, :], axis=0)}")
            print(f"Cov:     \n{np.cov(data[has_lbl, :], rowvar=False)}\n")
...
def heatmap_cov(data, headers, col_c=None, title="Covariance"):
    ...
    plt.setp(ax.get_yticklabels(), rotation=45, ha="right",
             rotation_mode="anchor")  # Rotate the column labels and set their alignment.
    fig.tight_layout()
...
def main(argv):
    ...
    if len(argv) > 2:
        try:
            ...
            class_col = headers.index(argv[
                                          2])  # CAUTION: this will crash if the user's input does not match any element in the list of headers
        except:
